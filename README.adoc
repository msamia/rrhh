...
Si obtenés *405 Method Not Allowed* o *404 Not Found*, confirmá que la lista `management.endpoints.web.exposure.include` contenga `circuitbreakers` y que la propiedad `management.endpoint.circuitbreakers.enabled` esté en `true` en `servicio-orquestador/src/main/resources/application.properties`. En este proyecto los endpoints se mapean explícitamente debajo de `/actuator/resilience4j/` mediante `management.endpoints.web.path-mapping.*` para evitar confusiones. Además se incluyó el controlador `CircuitBreakerStatusController` para exponer de forma explícita el estado de cada breaker en esa misma URL.
